...
def save_img(filename, outputdata, dt, rxnumber, rxcomponent, out_dir):
    ...
    fig.savefig(out_dir + os.sep + savefile + '_frequency' + '.png', format='png')
    plt.clf()
    plt.close()
    ...
    plt.imshow(
        ...
        vmin=-np.amax(np.abs(outputdata)),
        vmax=np.amax(np.abs(outputdata)))
    plt.savefig(out_dir + os.sep + savefile + '_post' + '.png', format='png')
    plt.clf()
    plt.close()

    fig = frequency_bscan(outputdata)
    fig.savefig(out_dir + os.sep + savefile + '_frequency_post' + '.png', format='png')
    plt.clf()
    plt.close()
